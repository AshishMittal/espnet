...
echo "${ngpu} gpus selected"
echo "${preprocess_config} is the preprocess config file"
echo "${train_config} is the train config file"
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    spm_train --input=data/lang_char/input.txt --vocab_size=${nbpe} --model_type=${bpemode} --model_prefix=${bpemodel} --hard_vocab_limit=false --input_sentence_size=100000000
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    if [ ! -e ${lmdatadir} ]; then
        ...
        #commbine with training set only.
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for rtask in ${recog_set}; do
    (
        decode_dir=decode_${rtask}_${recog_model}_$(basename ${decode_config%.*})_${decode_tag}
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}
